...
class DepthPointCloudPublisher(Node):
    def __init__(self):
        ...
        # kaair_obstacle의 Process.preprocess/downsample_voxel과 동일한 방식의
        # ROI PassThrough 필터 + Open3D voxel 다운샘플링 (카메라 광학 프레임 기준)
        ...
        self.get_logger().info(
            ...
            f"roi_x=[{self.roi_x_min:.2f},{self.roi_x_max:.2f}], "
            f"roi_y=[{self.roi_y_min:.2f},{self.roi_y_max:.2f}], "
    ...
    def _depth_callback(self, msg: Image):
        ...
        points_cam = self._apply_roi(points_cam)
        if points_cam.shape[0] == 0:
            return

        points_cam = self._downsample_voxel(points_cam)
        if points_cam.shape[0] == 0:
            return
    ...
    # ──────────────────────────────────────────────
    # Point cloud filtering (kaair_obstacle.utils.process.Process 참고)
    # ──────────────────────────────────────────────

    def _apply_roi(self, points_cam):
        """카메라 광학 프레임 기준 ROI PassThrough 필터 (x, y 좌우/상하 범위).

        깊이(z) 방향 범위는 _project_to_points 에서 min_range/max_range로
        이미 적용되었으므로 여기서는 x, y만 추가로 제한한다.
        """
        mask = (
            (points_cam[:, 0] >= self.roi_x_min) & (points_cam[:, 0] <= self.roi_x_max) &
            (points_cam[:, 1] >= self.roi_y_min) & (points_cam[:, 1] <= self.roi_y_max)
        )
        return points_cam[mask]

    def _downsample_voxel(self, points_cam):
        """Open3D voxel_down_sample로 다운샘플링 (voxel_size <= 0이면 통과)."""
        if self.voxel_size <= 0.0:
            return points_cam

        pcd = o3d.t.geometry.PointCloud()
        pcd.point.positions = o3d.core.Tensor(
            points_cam.astype(np.float32), dtype=o3d.core.float32
        )
        ds = pcd.voxel_down_sample(voxel_size=self.voxel_size)
        return ds.point.positions.numpy()
