#!/usr/bin/env python3
"""
Nav2 /spin 액션 클라이언트 예제 (nav2_msgs/action/Spin).

사전 조건: bt_navigator 등이 떠 있어 /spin 액션 서버가 있어야 함.
의존: ros-<distro>-nav2-msgs, builtin-interfaces

실행 예:
  python3 spin_relative.py --ros-args -p target_yaw:=1.57 -p time_allowance_sec:=30.0
"""
...
class SpinClient(Node):
    ...
    def send_goal_and_wait(self, timeout_sec: float = 120.0) -> bool:
        ...
        self.get_logger().info(
            f'Spin 목표: target_yaw={goal.target_yaw:.3f} rad '
            f'({math.degrees(goal.target_yaw):.1f} deg), '
        ...
        if status == 4:
            ...
            self.get_logger().info(
                f'Spin 성공. total_elapsed_time={elapsed.sec}.{elapsed.nanosec:09d} s'
            )
            return True
        self.get_logger().warn(f'Spin 종료 status={status} (4=성공)')
        return False
